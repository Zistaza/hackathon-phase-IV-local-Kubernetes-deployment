#!/usr/bin/env python3
"""
Simple test script to verify natural language commands are working properly
"""
...
agent_spec = importlib.util.spec_from_file_location("agent", os.path.join(parent_dir, "agents/todo_chatbot/agent.py"))
agent_module = importlib.util.module_from_spec(agent_spec)
agent_spec.loader.exec_module(agent_module)

intent_classifier_spec = importlib.util.spec_from_file_location("intent_classifier", os.path.join(parent_dir, "agents/todo_chatbot/intent_classifier.py"))
...
async def test_natural_language_commands():
    ...
    test_cases = [
        {
            ...
            "expected_intent": intent_classifier_module.IntentType.TASK_CREATION,
            "description": "Add a task to buy groceries"
        },
        {
            "input": "Show me all my tasks",
            "expected_intent": intent_classifier_module.IntentType.TASK_LISTING,
        ...
        },
        {
            ...
            "expected_intent": intent_classifier_module.IntentType.TASK_COMPLETION,
            "description": "Complete task 3"
        },
        {
            "input": "Delete the meeting task",
            "expected_intent": intent_classifier_module.IntentType.TASK_DELETION,
            "description": "Delete meeting task"
        },
        {
            "input": "Change task 1 to 'Call mom tonight'",
            "expected_intent": intent_classifier_module.IntentType.TASK_UPDATE,
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if intent == intent_classifier_module.IntentType.TASK_CREATION:
            if 'title' in extracted_params:
                print(f"   Task title: '{extracted_params['title']}'")
            else:
                print("   ❌ No title extracted for task creation")

        elif intent == intent_classifier_module.IntentType.TASK_COMPLETION:
        ...
        elif intent == intent_classifier_module.IntentType.TASK_LISTING:
